...
@synthesize delegate;
static FishBowlsdk* __strong instance = nil;
...
+(FishBowlsdk *)sharedInstanceWithAPIKey
{
    if(instance){
        ...
        instance =[[FishBowlsdk alloc]init];
        instance.isInRegion=false;
        instance.isFirstTimeAppLaunch=true;
       // instance.SERVER_URL = @"https://demo-fb.fishbowlcloud.com/";//demo
        instance.SERVER_URL = @"http://qa-jamba.fishbowlcloud.com/";//qa
                 ...
                 if ([responseObject isKindOfClass:[NSDictionary class]]) {
                     ...
                     if ([[storeresponse valueForKey:@"stores"] isKindOfClass:[NSArray class]]){
                         ...
                         for(NSDictionary* store in arraydic){
                             FishBowlStore *storeObj=[[FishBowlStore alloc]init];
...
-(void)saveCustomer:(FishBowlCustomer *)customerInfo :(void (^)(FishBowlCustomer *cusInfo, NSError *error))response {
   ...
   NSLog(@"CLPCustomer Save Customer");
    ...
    [manager POST:[SERVER_URL stringByAppendingString:@"clpapi/mobile/appcustomer"] parameters:[customerInfo toDictionary] success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            NSDictionary *customerresponse=(NSDictionary*)responseObject;
            currentCustomer=[[FishBowlCustomer alloc]init];
...
-(void)appCustomerForGuest
{
    ...
    [dict setValue:@"Guest" forKey:@"firstName"];
    ...
    {
        [dict setValue:strPushToken forKey:@"pushToken"];
    ...
    [manager POST:[SERVER_URL stringByAppendingString:@"clpapi/mobile/appcustomer"] parameters:dict success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            ...
            [[NSUserDefaults standardUserDefaults]setBool:YES forKey:@"callFirst"];
            [[NSUserDefaults standardUserDefaults]setBool:NO forKey:@"CurrentEvents"];
...
-(void)updateDeviceInSDK:(NSString *)customerID
{
    ...
    [manager.requestSerializer setValue:@"201969E1BFD242E189FE7B6297B1B5A5" forHTTPHeaderField:@"client_id"];
    [manager.requestSerializer setValue:@"C65A0DC0F28C469FB7376F972DEFBCB9" forHTTPHeaderField:@"client_secret"];
    [manager.requestSerializer setValue:@"1173" forHTTPHeaderField:@"access_token"];
...
//            [[NSUserDefaults standardUserDefaults]setValue:[customerresponse valueForKey:@""]forKey:@"Newmemberid"];
...
//        }
        if(alert){
            ...
            if(([ntype caseInsensitiveCompare:@"pass"]!=NSOrderedSame))
            {
                ...
                [delegate FishBowlPushDataBinding:promoCode withId:offerid];
            }
        }
...
-(void)getPass:(id)response
{
   // [obj removeLoadingView:self.view];
    ...
    FishBowlsdk * obj22 = [[FishBowlsdk alloc]init];
...
-(void)openPassbookAndShow:(NSURL*)url
{
    @try
    {
        [delegate FishBowlOpenPassbook];//starts loader
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, (unsigned long) NULL), ^{
            if(url==nil){
                [delegate FishBowlClosePassbook:@"No URL present to open a pass"];
                return;
            }
            ...
            if(error){
                ...
                [delegate FishBowlClosePassbook:error.debugDescription];
                return;
            }
            error = nil;
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if(error || pass==nil){
                    ...
                    [delegate FishBowlClosePassbook:@"Invalid Pass"];
                    ...
                    if([_passLib containsPass:pass]) {
                        ...
                        [delegate FishBowlClosePassbook:@"The pass you are trying to add to Passbook is already present."];
                    } else {
                       ...
                       // [delegate clpClosePassbook:@""];
                        ...
                        [vc setDelegate:nil];
                        if(vc!=nil){
                            UIViewController *currentviewctrl = [FishBowlsdk getTopController];
                            if(currentviewctrl){
...
-(void)updateLocation:(FishBowlMobileEvent *)event{
             ...
             if ([restResponse valueForKey:@"successFlag"]) {
                 ...
                 if ([[restResponse valueForKey:@"beacons"] isKindOfClass:[NSArray class]]){
                     ...
                     for(NSDictionary* beacon in arraydic){
                         ...
                         FishBowlBeacon *clpbeaconObj =[[FishBowlBeacon alloc]init];
...
- (void)HandleBeaconEnterRegion:(CLRegion *)region {
    ...
    if (storeBeacons != nil) {
        ...
        for (FishBowlBeacon* beacon in self.storeBeacons) {
...
-(void)locationManager:(CLLocationManager *)manager didRangeBeacons:(NSArray *)beacons
    ...
    FishBowlMobileEvent *beaconEvt = [[FishBowlMobileEvent alloc] init];
...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations{
    ...
    if(isInRegion && isTriggerGeoFence == YES){                                                   //send location update to server only when user in region
        FishBowlMobileEvent *locationEvent=[[FishBowlMobileEvent alloc]init];
    ...
    if(isFirstTimeAppLaunch && isInRegion){                            //Start Beacon Region Monitoring
        ...
        if (inRegionStoreList!=nil && inRegionStoreList.count!=0) {
            for (FishBowlStore *store in inRegionStoreList) {
...
-(void)locationManager:(CLLocationManager *)manager didEnterRegion:(CLRegion *)region{
    isInRegion=true;
    FishBowlStore *currentStore = nil;
    ...
    if (![[region identifier] isEqualToString: @"com.clp.beaconregion"]) {              //If location region entry
        ...
        if ((storeList != nil) && (storeList.count > 0)) {
            for (FishBowlStore *store in storeList) {                                        //Determine store for the entered region
...
-(void)checkInsideStoreRegion:(CLLocation*)userLocation{
    isInRegion=false;
    for(FishBowlStore *store in storeList){
...
-(void)stopMonitoringOutSideStoreRegion {
    for(CLRegion *monitored in [locationManager monitoredRegions]) {
        if(![[monitored identifier] isEqualToString:@"com.clp.beaconregion"]){          //Do not stop Beacon Region monitoring
            ...
            for (FishBowlStore *inRegionStore in inRegionStoreList) {
...
-(int)getNearestStoreDistance{
    if(nearestStoreList && nearestStoreList.count!=0){
        FishBowlStore *store=[nearestStoreList objectAtIndex:0];
...
- (void)getNearestStores:(CLLocation *)userLocation;
{
    ...
    sorted = [unsorted sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2)
              {
                  FishBowlStore *firstStore = (FishBowlStore *)obj1;
                  FishBowlStore *secondStore = (FishBowlStore *)obj2;
...
-(void)startLocationBasedRegionMonitoring{
    ...
    if([CLLocationManager locationServicesEnabled] && nearestStoreList!=nil && nearestStoreList.count!=0){
        ...
        for(FishBowlStore *store in nearestStoreList){
