...
class ProductDetailAvailableViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, ProductQuantityTableViewCellDelegate, MakeItLightTableViewCellDelegate, UITextFieldDelegate, SubstitutionsViewControllerDelegate {
    ...
    func updateQuantity() {
        ...
        if currentQuantity >= Constants.oloItemLimit {
            ...
            userChoice.quantity = 10 - currentQuantity
        }
            //Check if we have chosen 0 and we have room in basket. Then reset to at least 1.
        else if userChoice.quantity == 0 && currentQuantity < 10 {
    ...
    private func updateProductSizes() {
        ...
        for i in 0..<buttonCount {
            ...
            let sizeFont = UIFont(name: "Archer-Bold", size: 17)!//Actual size in psd is 20 but that looks very bad when name is large.
