...
- (void)drawText:(CGSize)rectSize context:(CGContextRef)c
{
    ...
    CGFloat valueFontSize = self.valueFontSize == -1 ? rectSize.height/3 : self.valueFontSize;
    ...
    BOOL isrewards=[[[NSUserDefaults standardUserDefaults]valueForKey:@"rewardsValue"]boolValue];
    if (isrewards==YES) {
        formatString = [NSString stringWithFormat:@"  %%.%df", (int)self.decimalPlaces];
    }
    else
    {
        formatString = [NSString stringWithFormat:@" %%.%df", (int)self.decimalPlaces];
    ...
    NSLog(@"textToPresent string ------ %@",textToPresent);
    ...
    if (decimalLocation != NSNotFound){
        ...
        [text setAttributes:valueDecimalFontAttributes range:decimalRange];
         ...
         NSLog(@"text string ------ %@",text);
    }
    
    // ad the unit only if specified
//    if (self.showUnitString) {
//        NSDictionary* unitFontAttributes = @{NSFontAttributeName: [UIFont fontWithName: self.unitFontName size:self.unitFontSize == -1 ? rectSize.height/7 : self.unitFontSize], NSForegroundColorAttributeName: [UIColor blackColor], NSParagraphStyleAttributeName: textStyle};
//        
...
//    }
    ...
    CGPoint textCenter = CGPointMake(
                                     ...
                                     rectSize.height/2-percentSize.height/1.5 + self.textOffset.y+20
                                     );
