...
class StoreService: NSObject {
    ...
    private class func findStoresByLocation(location: CLLocation, searchType: StoreSearchResultType, callback: (stores: StoreList, error: NSError?) -> Void) {
        ...
        OloRestaurantService.restaurantsNear(searchlocation.coordinate.latitude, longitude: searchlocation.coordinate.longitude, radius: searchRadiusInMiles, limit: searchResultsLimit) { (restaurants, error) in
            ...
            // add demo vendor restaurant
   //         if Configuration.sharedConfiguration.ShowDemoVendor {
   //             restaurants = addDemoVendor(restaurants)
   //         }
            
            oloStores = restaurants
            log.verbose("Olo Stores: \(oloStores.count)")
            dispatch_group_leave(group)
        }


                // add demo vendor store
       //         if Configuration.sharedConfiguration.ShowDemoVendor {
    //                stores = addSpendgoDemoVendor(stores)
      //          }
...
//    
//    // Demo Vendor
//    class func addDemoVendor(var stores: OloRestaurantList!) -> OloRestaurantList{
//        let filePath = NSBundle.mainBundle().pathForResource("olo-demo-vendor", ofType: "json")
//        let value = NSFileManager.defaultManager().contentsAtPath(filePath!)
//        guard let jsonObject = try? NSJSONSerialization.JSONObjectWithData(value!, options: NSJSONReadingOptions()) else {
//            return stores
//        }
//        
//        let json = JSON(jsonObject)
//        let store = OloRestaurant(json: json)
//        stores.append(store)
//        return stores
//    }
//    
//    class func addSpendgo.Vendor(var stores: [SpendGoStore]!) -> [SpendGoStore]{
//        let filePath = NSBundle.mainBundle().pathForResource("spendgo-demo-vendor", ofType: "json")
//        let value = NSFileManager.defaultManager().contentsAtPath(filePath!)
//        guard let jsonObject = try? NSJSONSerialization.JSONObjectWithData(value!, options: NSJSONReadingOptions()) else {
//            return stores
//        }
//        
//        let json = JSON(jsonObject)
//        let store = SpendGoStore(json: json)
//        stores.append(store)
//        return stores
//    }
//    //
