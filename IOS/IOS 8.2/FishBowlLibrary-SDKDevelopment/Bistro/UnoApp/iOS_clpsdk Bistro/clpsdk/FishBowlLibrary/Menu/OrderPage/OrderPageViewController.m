...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    _headerView.layer.borderColor=[UIColor blackColor].CGColor;
        ...
        if (isFromorder != [[[NSUserDefaults standardUserDefaults]valueForKey:@"isFromrderComplete"]boolValue]) {
            ...
            _shoeCity.text=[userDefaults valueForKey:@"city"];
            _showAddress.text=[userDefaults valueForKey:@"address"];
...
-(void)calCulatePrice
{
    ...
    NSLog(@"total price cost -------- %@",_totalCost.text);
    
     NSLog(@"obj.itemDictArray value ------- %@",obj.itemDictArray);
...
- (IBAction)continueClicked:(id)sender
{
    ...
    if(obj.itemDictArray.count>0)
    {
        ...
        else
        {
            ...
            BOOL fromMenuToStore=[[[NSUserDefaults standardUserDefaults]valueForKey:@"MenuToStore"]boolValue];
            if (fromMenuToStore==YES) {
                 OrderCompleteViewController* SubMenuObj = [[OrderCompleteViewController alloc]initWithNibName:@"OrderCompleteViewController" bundle:nil];
        ...
        [self.navigationController pushViewController:SubMenuObj animated:YES];
            ...
            else
            {
                ...
                StorelocatorViewController* SubMenuObj = [[StorelocatorViewController alloc]initWithNibName:@"StorelocatorViewController" bundle:nil];
...
-(void)removeOrderClicked:(id)sender
{
      ...
      NSLog(@"itemDictArray addNew value --------------%@",obj.itemDictArray);
    ...
    UIAlertAction *cancel1 = [UIAlertAction actionWithTitle:@"YES" style:UIAlertActionStyleCancel handler:^(UIAlertAction  *action) {
        ...
        if (indexPath)
        {
            ...
            NSLog(@"itemDictArray remove value --------------%@",obj.itemDictArray);
