...
public class PushDetail_Activity extends Activity
{
    ...
    protected void onStart() {
        super.onStart();
        if(FBThemeMobileSettingsService.sharedInstance().generalmapsetting.get("GeneralButtonType1NormalColor")!=null)
        {
            mImageLoader = CustomVolleyRequestQueue.getInstance(this.getApplicationContext()).getImageLoader();
            final String url2 = "http://" + FBThemeMobileSettingsService.sharedInstance().registermapsetting.get("SignUpHeaderImageUrl");
            mImageLoader = CustomVolleyRequestQueue.getInstance(this.getApplicationContext()).getImageLoader();
