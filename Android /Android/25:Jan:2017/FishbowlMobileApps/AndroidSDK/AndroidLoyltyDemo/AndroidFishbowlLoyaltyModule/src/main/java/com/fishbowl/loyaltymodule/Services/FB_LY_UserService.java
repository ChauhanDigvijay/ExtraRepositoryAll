...
import com.fishbowl.loyaltymodule.Models.FBStateItem;
...
public class FB_LY_UserService {
    ...
    public List<FBStateItem> allStateListfromServer = new ArrayList<FBStateItem>();
...
//		  	       // getClient().get(context, SERVER_URL + "mobile/getofferforcustomer" + "/" + customerId + "/" + "0", entity, "application/json", new JsonHttpResponseHandler() {
    ...
    public void getStateUpdate(JSONObject parameter,final FBStateCallback callback){
        //  final FBSdkData FBSdkData = clpsdk.getClpSdkData();
                    ...
                    if (error == null && response != null) {
                        ...
                        if (getArrayStates != null) {
                            ...
                            for (int i = 0; i < getArrayStates.length(); i++) {
                                ...
                                FBStateItem statesListObj = new FBStateItem(myStoresObj);
                                allStateListfromServer.add(statesListObj);
                                mapStateNumToId.put(statesListObj.getStateCode(), statesListObj.getStateName());
                            }
