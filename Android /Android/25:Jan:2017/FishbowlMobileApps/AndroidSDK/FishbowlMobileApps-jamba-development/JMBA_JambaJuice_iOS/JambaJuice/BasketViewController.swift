...
class BasketViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, BasketOrderTimeTableViewCellDelegate, MGSwipeTableCellDelegate {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if basket.products.count == 0 && indexPath.row == 0 {
                ...
                return tableView.dequeueReusableCellWithIdentifier("BasketAddAnotherItemCell") as! BasketAddAnotherItemCell
            }
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if indexPath.section == 0 {
            if let cell = tableView.cellForRowAtIndexPath(indexPath) as? BasketItemTableViewCell {
                cell.showSwipe(.RightToLeft, animated: true)
            } else if (tableView.cellForRowAtIndexPath(indexPath) as? BasketAddAnotherItemCell) != nil {
                dismiss()
            }
        }
        if indexPath.section == 3 {
