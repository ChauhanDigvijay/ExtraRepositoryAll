...
class ClpPromotionService{
    ...
    class func getPromoCode(customerId : String,offerId : String, callback: ClpPromotionServiceCallback){
        ...
        ClpService.get("\(AppConstants.URL.AppPointingURL(intPointingServer))clpapi/mobile/getPromo/\(customerId)/\(offerId)",parameters: parameters){(response: JSON, error: NSError?)-> Void in
