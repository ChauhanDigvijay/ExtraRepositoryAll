...
typealias InCommAutoReloadDeleteCallBack = (error: NSError?) -> Void
...
class PaymentViewController: UIViewController,UITableViewDelegate {
    ...
    enum paymentDetailFieldName : String {
        ...
        case street1        = "Street Address 1"
        case street2        = "Street Address 2"
        case city           = "Select City"
    ...
    override func viewDidLoad() {
        //get country code for purchser details
        countryCode = creditCardDetail!.country
        getStateNameAndCountryName()
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if (indexPath.row == paymentDetailFieldName.cardType.hashValue) {
            let cell = tableView.dequeueReusableCellWithIdentifier("PaymentImageTableViewCell", forIndexPath: indexPath) as! PaymentImageTableViewCell
            
            cell.setCellData(name_label_text[indexPath.row], imageURL: cardTypeImageURL)
            
            return cell
        } else {
            let cell = tableView.dequeueReusableCellWithIdentifier("PaymentLabelTableViewCell", forIndexPath: indexPath) as! PaymentLabelTableViewCell
            ...
            if (name_label_text[indexPath.row] == paymentDetailFieldName.country.rawValue && countryName != "") {
                cell.details.text = countryName
            } else if (name_label_text[indexPath.row] == paymentDetailFieldName.state.rawValue && countryName != "") {
                cell.details.text = stateName
            }
            return cell
        }
    }
    ...
    func getAutoReloadConfigGiftCardsForPaymentAccountId(){
        SVProgressHUD.showWithStatus("Processing...", maskType: .Clear)
    ...
    func deleteAutoReload(deleteAutoReloadDetails:[CardAutoReloadDetails]){
        ...
        self.deleteAutoReload(deleteDetail) { (error) in
            if error != nil{
                ...
                autoReloadDetails.removeAtIndex(index)
    ...
    func deleteAutoReload(deleteAutoReloadDetails:CardAutoReloadDetails, callback:InCommAutoReloadDeleteCallBack){
        ...
        SVProgressHUD.showWithStatus("Removing auto reload config from \(autoReloadCardName)", maskType: .Clear)
    ...
    func deleteCreditCard(){
        SVProgressHUD.showWithStatus("Deleting Credit Card...", maskType: .Clear)
    ...
    //MARK: - ConfirmExit
    func confirmExit(error:NSError!){
        SVProgressHUD.dismiss()
    ...
    //MARK: - Get country & state Name
    func getStateNameAndCountryName() {
        let brand = InCommGiftCardBrandDetails.sharedInstance.brand
        for country in brand!.billingCountries {
            if (country.code == creditCardDetail?.country) {
                countryName = country.name
                break
            }
        }
        //state full code countrycode-statecode  e.g:US-OR
        let stateFullCode = (creditCardDetail?.country)! + "-" + (creditCardDetail?.stateProvince)!
        for state in brand!.billingStates {
            if (stateFullCode == state.code) {
    ...
    func getMonthInText(cardMonthyValue:UInt16) -> String {
