...
class PickerViewController: UIViewController,UIPickerViewDelegate {
    ...
    @IBAction func savePickerValue() {
        if (noOfComponents == 1) {
            let res:String = "\(pickerData[0][picker.selectedRowInComponent(0)])"
            delegate?.pickerValueChanged(res,index: picker.selectedRowInComponent(0))
        } else if (noOfComponents == 2){
            let res:String = "\(pickerData[0][picker.selectedRowInComponent(0)]) - \(pickerData[1][picker.selectedRowInComponent(1)])"
            delegate?.pickerValueChanged(res,index: picker.selectedRowInComponent(0))
        }
        closePicker()
    }
