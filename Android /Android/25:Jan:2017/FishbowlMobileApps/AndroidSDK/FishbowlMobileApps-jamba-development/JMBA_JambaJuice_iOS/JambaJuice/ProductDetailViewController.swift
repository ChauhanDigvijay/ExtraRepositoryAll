...
class ProductDetailViewController: ScrollableModalViewController, ProductDetailAvailableViewControllerDelegate, SelectStoreViewControllerDelegate, ProductDetailPickerViewControllerDelegate {
    ...
    private var productDetailPickerViewController: ProductDetailPickerViewController?
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "ProductAvailableSegue" {
            ...
            productDetailPickerViewController = segue.destinationViewController as? ProductDetailPickerViewController
    ...
    @IBAction func addToBasket(sender: UIButton) {
        ...
        //                optionsAsInt64Ids.insert(optionId.choiceId)
        //            }
        //        }
        ...
        //            isAppEvent = true
        //            clpAnalyticsService.sharedInstance.clpTrackScreenView("ProductAdded");
        //            }
        //
    ...
    func showProductQuantityPickerView(selectedValue:String, parentModifier: StoreMenuProductModifier, option: StoreMenuProductModifierOption) {
        ...
        if var maxValue = parentModifier.maxAggregateQuantity {
            ...
            let totalQuanitySelected = productAvailableViewController?.userChoice.getTotalQuantityWithOptionId(parentModifier.id, optionId: option.id)
    ...
    @IBAction func showProductSearchScreen(){
        if screenPresented {
            performSegueWithIdentifier("ShowProductSearchVC", sender: self)
        } else {
            dismissModalController()
        }
    }
