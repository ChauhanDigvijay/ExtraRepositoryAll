...
class AddExistingGiftCardViewController: UIViewController,UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(AddExistingGiftCardViewController.keyBoardWillShown(_:)), name: UIKeyboardWillShowNotification, object: nil)
        
        NSNotificationCenter.defaultCenter().addObserver(self, selector:#selector(AddExistingGiftCardViewController.keyBoardWillHidden(_:)), name: UIKeyboardWillHideNotification, object: nil)
    ...
    @IBAction func checkBalanceButtonPressed(sender:UIButton){
    ...
    func validateGiftCardExistsOrNot(){
        if(giftCardNumberTextField.text == nil || giftCardNumberTextField.text!.isEmpty){
            ...
            SVProgressHUD.showWithStatus("Getting Card Details...", maskType: .Clear)
    ...
    func validateGiftCardNumber() -> Bool{
        ...
        return regexText.evaluateWithObject(giftCardNumberTextField.text)
    }
    
    // MARK: - Validate Gift Card Pin Number
    func validateGiftCardPinNumber() -> Bool{
        let regexText = NSPredicate(format: "SELF MATCHES %@", "^[0-9]+$")
        return regexText.evaluateWithObject(giftCardPinNumberTextField.text)
    }
    
    // MARK: - Prepare for segue
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "ExistingGiftCardDetailsSegue"{
            let destinationViewController = segue.destinationViewController as! ExistingGiftCardDetailsViewController
