...
class MyRewardsTableViewController: UITableViewController,clpSdkDelegate, OfferServiceDelegate {
    ...
    override func viewDidLoad() {
        ...
        //        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(MyRewardsTableViewController.showOffersList), name: "offersListUpdated", object: nil)
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, triggerTime), dispatch_get_main_queue(), { () -> Void in
            ...
            if appdelegate?.isPushOpen == false
            {
                self.showOffersList()
            }
        })
        
        self.refreshControl?.addTarget(self, action: #selector(MyRewardsTableViewController.getOfferList), forControlEvents: .ValueChanged)
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0
        {
            ...
            if offers.count > 0 {
                ...
                if indexPath.row == 0
                {
                    let cell = tableView.dequeueReusableCellWithIdentifier("OfferInstructionsTableViewCell")!
