...
struct StoreMenuProductModifier {
    ...
    init (oloModifier: OloModifier) {
        ...
        else if modifierName.lowercaseString.rangeOfString("Select Protein Boost:") != nil {
    ...
    func isModifierNameIsAddYummyExtras() ->Bool {
        if (name.lowercaseString.hasPrefix("add yummy extras")) {
            return true
        }
        return false
    }
    ...
    func isModifierNameIsCustomizeIt() ->Bool {
        if (name.lowercaseString.hasPrefix(customizeModifierText)
            ||
            (name.lowercaseString.hasPrefix(customizeModifierTextSandbox))
            ) {
            return true
        }
        return false
    ...
    func isAddToppingModifier() -> Bool {
        return name.lowercaseString.hasPrefix("add topping")
    }
}
