...
struct GiftCardAppConstants {
    ...
    //MARK: Genral Error
    static let generalError:NSError = NSError.init(description: "Unable to proceed please Retry")
    ...
    static let addNewPaymentTitle = "Add New Payment"
    ...
    //gift card
    static let isiPhone = (UIDevice.currentDevice().userInterfaceIdiom == .Phone)
    ...
    // MARK: Date to Send
    static func getDateToSend(dateToSend:NSDate) -> String{
        let dateFormatter = NSDateFormatter()
        dateFormatter.dateFormat = GiftCardAppConstants.ShortDateFormat
        let selectedDate = dateToSend
        let now = NSDate()
        let calendar = NSCalendar(calendarIdentifier: NSCalendarIdentifierGregorian)!
        let components = calendar.components(.Day, fromDate: selectedDate, toDate: now, options: .WrapComponents)
        let days = components.day
        var dateToSendText = dateFormatter.stringFromDate(selectedDate)
    ...
    static func nameValidation(name:String) -> Bool{
        ...
        if regexText.evaluateWithObject(name){
            if name.length > 20{
                return false
            }else{
                return true
