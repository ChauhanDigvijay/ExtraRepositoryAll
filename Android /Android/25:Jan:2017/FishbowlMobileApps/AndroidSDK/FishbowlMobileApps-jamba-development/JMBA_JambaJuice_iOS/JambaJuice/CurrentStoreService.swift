...
class CurrentStoreService{
    ...
    func loadAds(callback: AdProductListCallback) {
        ...
        if let result = try? query.findObjects() {
            ...
            callback(AdClass.init(), NSError(description: "failed to load products"))
        } else {
            callback(AdClass.init(), NSError(description: "failed to load products"))
        }
    }
    ...
    private func adDetailsForAd(ad: AdClass) -> AdDetailList {
