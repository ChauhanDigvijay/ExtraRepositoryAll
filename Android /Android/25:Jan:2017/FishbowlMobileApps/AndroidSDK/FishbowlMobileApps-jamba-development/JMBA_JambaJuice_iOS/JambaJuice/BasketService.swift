...
class BasketService: NSObject {
    ...
    class func addProductWithOptionQuantity(product: Product, quantity: Int64, options: [OloProductChoiceQuantity], specialInstructions: String?, callback: BasketCallback) {
        if sharedBasket == nil {
        ...
        if existingQuantity + quantity >  Constants.oloItemLimit {
            callback(basket: nil, error: NSError(description: "Can not add more than \(Constants.oloItemLimit) items in your cart per order."))
            return
        }
    ...
    class func placeOrderWithGiftCard(giftCard: InCommUserGiftCard, callback: PlaceOrderCallback){
        if sharedBasket == nil {
            callback(orderStatus: nil, error: NSError(description: "Basket not found."))
            return
        }
