...
//    }
    ...
    public static void getBillingAccounts(final BillingAccountsCallback billingAccountsCallback){
        OloUserService.getBillingAccount(DataManager.getInstance().getCurrentBasket().getId(),new OloBillingAccountCallback() {
            @Override
            public void onBillingAccountCallback(OloBillingAccount[] accounts, Exception error) {
                ArrayList<BillingAccount> billingAccounts = new ArrayList<BillingAccount>();
                if(accounts != null && accounts.length > 0){
                    for(OloBillingAccount oloBillingAccount: accounts){
                        BillingAccount billingAccount = new BillingAccount(oloBillingAccount);
                        billingAccounts.add(billingAccount);
                    }
                }

                if(billingAccountsCallback != null){
