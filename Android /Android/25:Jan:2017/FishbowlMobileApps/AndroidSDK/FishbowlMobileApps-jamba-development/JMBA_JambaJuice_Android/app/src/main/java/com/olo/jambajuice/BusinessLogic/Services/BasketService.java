...
import com.wearehathway.apps.olo.Models.OloBasketProductBatchError;
...
public class BasketService {
    ...
    public static void addProductToBasket(final Activity activity, String basketId, final BasketProduct product, final BasketServiceCallback callback) {
...
//                }
                ...
                OloBasket oloBasket = null;
                if (result != null) {
                    ArrayList<OloBasketProductBatchError> errors = result.getErrors();
                    oloBasket = result.getBasket();
                    if(errors != null && errors.size() > 0){
                        String msg = "";
                        if(errors.get(0) != null && StringUtilities.isValidString(errors.get(0).getMessage())){
                            msg = errors.get(0).getMessage();
                        }
                        updateBasketAndNotify(oloBasket, callbackWeakReference, new Exception(msg), callback);
                    }
                    else if (error == null) {
...
//                            , FBEventSettings.ADD_PRODUCT);
    ...
    public static void placeOrder(Activity activity, final PlaceOrderCallback callback) {
        ...
        if (basket == null) {
            ...
            JambaAnalyticsManager.sharedInstance().track_ItemWith(basket.getId(), "", FBEventSettings.SUBMIT_ORDER);
        }
        final WeakReference<Activity> callbackWeakReference = new WeakReference<Activity>(activity);
...
//                                if(DataManager.getInstance().getBillingAccount()==null) {
//                                    billingAccounts = new ArrayList<BillingAccount>();
//                                }else{
//                                    billingAccounts = new ArrayList<BillingAccount>(DataManager.getInstance().getBillingAccount());
//                                }
...
//                            }else if (oloBillingScheme.getType().equals("giftcard")){
//                                ArrayList<OloBillingAccount> oloBillingAccounts = oloBillingScheme.getAccounts();
//                                if(DataManager.getInstance().getBillingAccount()==null) {
//                                    billingAccounts = new ArrayList<BillingAccount>();
//                                }else{
...
//                                    {
//                                        BillingAccount billingAccount = new BillingAccount(oloBillingAccount);
//                                        billingAccount.setCardtype("Jamba Cards");
//                                        billingAccounts.add(billingAccount);
//                                    }
//                                }
//                                DataManager.getInstance().setBillingAccount(billingAccounts);//previous gift card list
//                                DataManager.getInstance().setCurrentBasketSupportGiftCard(true);//supporting gift card
//                            }
//
//                        }
//                    }
...
//        } else {
        ...
        OloBasketService.getBillingSchemes(basketId, new OloBillingSchemeServiceCallback() {
            ...
            public void onBillingSchemeServiceCallback(ArrayList<OloBillingScheme> oloBillingSchemes, Exception error) {
                ArrayList<BillingAccount> billingAccounts = null;
//                UNCOMMENT WHEN WE HAVE TO SHOW SAVED GIFT CARDS TOO
                if (oloBillingSchemes != null) {
                    billingAccounts = new ArrayList<BillingAccount>();
                    for (OloBillingScheme oloBillingScheme : oloBillingSchemes) {
                        if (oloBillingScheme.getType().equals("creditcard")
                                || oloBillingScheme.getType().equals("payinstore")
                                || oloBillingScheme.getType().equals("giftcard")) {
                            ArrayList<OloBillingAccount> oloBillingAccounts = oloBillingScheme.getAccounts();
                            if (oloBillingAccounts != null) {
                                for (OloBillingAccount oloBillingAccount : oloBillingAccounts) {
                                    BillingAccount billingAccount = new BillingAccount(oloBillingAccount);
                                    if (oloBillingScheme.getType().equals("giftcard")) {
                                        billingAccount.setCardtype("Jamba Cards");
                                    }
                                    billingAccounts.add(billingAccount);
                                }
...
//                UNCOMMENT WHEN WE HAVE TO SHOW only SAVED CREDIT CARDS
//                if (oloBillingSchemes != null) {
//                    billingAccounts = new ArrayList<BillingAccount>();
//                    for (OloBillingScheme oloBillingScheme : oloBillingSchemes) {
//                        if (oloBillingScheme.getType().equals("creditcard")
...
//                }
    ...
    private static void setTimeWanted(final BasketServiceCallback callback) {
        ...
        if (timeWanted == null) {
            JambaAnalyticsManager.sharedInstance().track_ItemWith(currentBasket.getId(), "ASAP", FBEventSettings.PICK_UP_TIME);
            ...
            JambaAnalyticsManager.sharedInstance().track_ItemWith(basketId, todayOrTomorrow + ":" + time.replace("am", "AM").replace("pm", "PM"), FBEventSettings.PICK_UP_TIME);
...
//                        String time = hour + ":" + minute;
    ...
    public static void addUpSell(JSONObject jsonObject,final BasketServiceCallback callback) {
        String basketId = DataManager.getInstance().getCurrentBasket().getId();
       OloBasketService.addUpsell(basketId, jsonObject, new OloBasketServiceCallback() {
           @Override
           public void onBasketServiceCallback(OloBasket basket, Exception error) {
               if (basket != null) {
                   DataManager manager = DataManager.getInstance();
                   manager.updateBasket(basket);
               }
               callback.onBasketServiceCallback(DataManager.getInstance().getCurrentBasket(),error);
           }
       });
    ...
    public static void validateBasket(final BasketValidationCallback callback) {
        ...
        OloBasketService.validateBasket(currentBasket.getId(), new OloBasketValidationCallback() {
            ...
            public void onOloBasketValidationCallback(OloBasketValidation oloBasketValidation, Exception e) {
                callback.onBasketValidated(oloBasketValidation,e);
            }
        });
    }
        ...
        setTimeWanted(new BasketServiceCallback() {
            ...
            public void onBasketServiceCallback(Basket basket, Exception e) {
                if (e == null) {
                    ...
                    callback.onBasketValidated(null,e);
                }
            }
        });
    }
    ...
    public static void removeRewards(final BasketServiceCallback callback) {
        ...
        if (rewards.size() > 0) {
            ...
            if (appliedReward.getRewardId() == 0) {
                ...
                JambaAnalyticsManager.sharedInstance().track_ItemWith("", "REWARD_TITLE:" + appliedReward.getRewardTitle() + ";MEMBERSHIP_ID:" + appliedReward.getMembershipId() + ";REFERENCE:" + appliedReward.getReference(), com.fishbowl.basicmodule.Analytics.FBEventSettings.REMOVE_REWARD);
            }
    ...
    private static OloBasketProduct convertToOloBasketProduct(BasketProduct basketProduct) {
        ...
        oloBasketProduct.setSpecialinstructions(basketProduct.getSpecialinstructions());
...
//                                        String json = gson.toJson(store);
    ...
    public static void cancelOrder(Activity activity, final RecentOrder recentOrder, final OloOrderServiceCallback callback) {
        final WeakReference<Activity> callbackWeakReference = new WeakReference<Activity>(activity);
        OloBasketService.cancelOrder(recentOrder.getId(), new OloOrderServiceCallback() {
            @Override
