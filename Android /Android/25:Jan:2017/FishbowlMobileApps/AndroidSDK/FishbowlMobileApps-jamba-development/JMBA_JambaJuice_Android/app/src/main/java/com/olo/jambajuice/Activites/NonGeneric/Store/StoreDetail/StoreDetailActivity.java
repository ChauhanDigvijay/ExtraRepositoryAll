...
//            startOrder.setVisibility(View.GONE);
    ...
    private void addOpenTimings(StoreTiming calendar) {
        ...
        ArrayList<TimeRange> firstSevenDaysRanges = new ArrayList<TimeRange>(ranges);
        if(ranges.size() > 7) {
            firstSevenDaysRanges.clear();
            firstSevenDaysRanges = new ArrayList<TimeRange>(ranges.subList(0, 7));
        }
        ...
        for (String dayName : weekDaysName) {
            ...
            for (TimeRange timeRange : firstSevenDaysRanges) {
