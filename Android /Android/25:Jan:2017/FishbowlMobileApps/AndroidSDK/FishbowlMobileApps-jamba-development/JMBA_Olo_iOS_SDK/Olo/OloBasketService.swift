...
public typealias OloBasketTransferCallback = (basketTransfer: OloBasketTransfer?, error: NSError?) -> Void
...
public class OloBasketService {
    ...
    public class func basketTransfer(vendorId: Int64, basketId:String, callback: OloBasketTransferCallback){
        var parameters: [String : AnyObject] = [
            "vendorid": NSNumber(longLong: vendorId)
        ]
        if let authToken = OloSessionService.authToken {
        ...
        OloService.post("/baskets/\(basketId)/transfer", parameters: parameters) { (response, error) -> Void in
            if  error != nil {
                callback(basketTransfer: nil, error: error)
                return
            }
            callback(basketTransfer: OloBasketTransfer(json: response), error: nil)
        }
    }
    ...
    public class func addProductsWithChoiceQuantity(basketId: String, newBasketProducts: [OloProductWithChoiceQuantity],callback: OloBasketProductBatchResultCallback) {
        let parameters = ["products": newBasketProducts.map { $0.serializeAsJSONDictionary() }]
        OloService.post("/baskets/\(basketId)/products/batch", parameters: parameters) { (response, error) -> Void in
            if error != nil {
                callback(result: nil, error: error)
