...
class OloUserTests: OloTestCase {
    ...
    func helperUserDictionaryArrayWithCount(_ count: UInt64) -> [[String: AnyObject]] {
    ...
    func helperUserDictionaryWithIndex(_ index: UInt64) -> [String: AnyObject] {
        var dict: [String: AnyObject] = [
            "firstname": "firstName\(index)" as AnyObject,
            "lastname": "lastName\(index)" as AnyObject,
            "emailaddress": "email\(index)@email.com" as AnyObject
        ]
        if (index % 2) == 0 {
            dict["authtoken"] = "authToken\(index)" as AnyObject?
            dict["cardsuffix"] = "cardsuffix\(index)" as AnyObject?
        }
        if (index % 2) == 1 {
            dict["basketid"] = "basketId\(index)" as AnyObject?
            dict["contactnumber"] = "11122233\(index)" as AnyObject?
        }
        if (index % 3) == 0 {
            dict["reference"] = "reference\(index)" as AnyObject?
        }
        return dict
    }
    
    func helperCompareUserWithDictionary(_ user: OloUser, dictionary:[String: AnyObject]) {
