...
class OloBasketProductBatchResultTests: OloTestCase {
    ...
    func testInitWithJSON() {
        ...
        dict["basket"] = OloBasketTests.helperBasketDictionaryWithIndex(1) as AnyObject?
        var errorsDictArray: [[String: AnyObject]] = []
        for i in 0..<2 {
            var errorDict: [String: AnyObject] = [:]
            errorDict["productid"] = (i + 1) as AnyObject
            errorDict["message"] = "message\(i + 1)" as AnyObject?
            errorsDictArray.append(errorDict)
        }
        dict["errors"] = errorsDictArray as AnyObject?
        let json = JSON(dict)
