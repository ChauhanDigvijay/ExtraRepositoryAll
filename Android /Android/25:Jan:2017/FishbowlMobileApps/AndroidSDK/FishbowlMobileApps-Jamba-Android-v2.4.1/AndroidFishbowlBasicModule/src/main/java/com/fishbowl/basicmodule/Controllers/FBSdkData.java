...
public class FBSdkData implements Serializable {
	...
	public List<FBStoreItem> allCLPBeaconStoreList;
	public List<FBStoreItem> allStoresList;
	public FBMobileSettingItem mobileSettings;
	public FBEventSettings FBEventSettings;
	...
	public FBSdkData(Context context, String fileName) {
		...
		this.mobileSettings = new FBMobileSettingItem();
	...
	public void save() {
		try {
			...
			FBSdk.sharedInstance(context).localLog(getClass().getSimpleName(),
		...
		try {
			...
			if (key.equals(FBSdk.PERSISTENT_CONTEXT)
				...
				if (dataList != null && dataList.size() > 0) {
					this.allCLPBeaconStoreList = new ArrayList<FBStoreItem>();
					for (int i = 0; i < dataList.size(); i++) {
						this.allCLPBeaconStoreList.add((FBStoreItem) dataList
								.get(i));// all beacon
						                 // stores
					}
				}
				...
				List<?> dataList = (List<?>) dataObj;
				if (dataList != null && dataList.size() > 0) {
					this.allStoresList = new ArrayList<FBStoreItem>();
					for (int i = 0; i < dataList.size(); i++) {
						this.allStoresList.add((FBStoreItem) dataList.get(i));// store
																			// store
																			// list
					}
				}
					...
					&& dataObj instanceof FBEventSettings) {
				FBEventSettings dataEventSettings = (FBEventSettings) dataObj;
	...
	public List<FBStoreItem> getAllCLPBeaconStoreList() {
		if (this.allCLPBeaconStoreList == null) {
			FBSdk.sharedInstance(context).localLog("Clpdata :",
	...
	public List<FBStoreItem> getAllStoresList() {
	...
	public void setMobileSettings(FBMobileSettingItem mobileSettings) {
		save(FBSdk.PERSISTENT_MOBILE_SETTINGS, mobileSettings);
	}
	
	public FBEventSettings getEventSetting() {
		return FBEventSettings;
	}

	public void setEventSetting(FBEventSettings eventSetting) {
