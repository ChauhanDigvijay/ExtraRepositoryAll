...
class InCommSubmitPaymentHelper {
    ...
    class func createSubmitPaymentWithDiffrentiatingNumber(_ num: Int) -> InCommSubmitPayment {
        //Payment
    ...
    class func testSubmitPayment(_ submitPayment: InCommSubmitPayment) {
        testSubmitPaymentJSONDict(submitPayment.serializeAsJSONDictionary(), submitPayment: submitPayment)
    }
    
    class func testSubmitPaymentJSONDict(_ submitPaymentJSONDict: InCommJSONDictionary, submitPayment: InCommSubmitPayment) {
        ...
        XCTAssertEqual(submitPayment.creditCardExpirationMonth!, (submitPaymentJSONDict["CreditCardExpirationMonth"] as? NSNumber)?.uint16Value, "CreditCardExpirationMonth should have been same")
        XCTAssertEqual(submitPayment.creditCardExpirationYear!, (submitPaymentJSONDict["CreditCardExpirationYear"] as? NSNumber)?.uint16Value, "CreditCardExpirationYear should have been same")
        ...
        XCTAssertEqual(submitPayment.creditCardType!,submitPaymentJSONDict["CreditCardType"] as? String, "CreditCardType should have been same")
        XCTAssertEqual(submitPayment.paymentAccountId!, (submitPaymentJSONDict["PaymentAccountId"] as? NSNumber)?.int32Value, "PaymentAccountId should have been same")
