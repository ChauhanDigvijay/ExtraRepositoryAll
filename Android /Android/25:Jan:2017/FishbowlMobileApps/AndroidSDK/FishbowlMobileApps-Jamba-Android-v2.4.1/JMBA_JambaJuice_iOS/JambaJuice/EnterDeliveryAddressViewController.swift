    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if index == 1 || index == 5 {
            cell.textField.attributedPlaceholder = NSAttributedString(string: fieldName[index],
                                                                      attributes: [NSForegroundColorAttributeName: UIColor(hex: Constants.jambaGrayColor)])
            cell.textField.textColor = UIColor(hex: Constants.jambaMediumGrayColor)
        } else {
            cell.textField.attributedPlaceholder = NSAttributedString(string: fieldName[index],
                                                                      attributes: [NSForegroundColorAttributeName: UIColor(hex: Constants.jambaDarkGrayColor)])
            cell.textField.textColor = UIColor(hex: Constants.jambaMediumGrayColor)
...
extension String {
    func formatPhoneNumber() -> String {
        ...
        if formattedNumber.characters.count >= 10 {     //check maximum limit
            ...
            formattedNumber = formattedNumber.substring(to: index)
        }
