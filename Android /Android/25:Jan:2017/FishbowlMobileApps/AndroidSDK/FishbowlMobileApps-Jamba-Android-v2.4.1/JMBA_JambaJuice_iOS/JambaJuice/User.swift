...
class User: Equatable {
    ...
    var recentOrders: [OrderStatus]? {
        didSet {
            NotificationCenter.default.post(name: Notification.Name(rawValue: JambaNotification.RecentOrdersUpdated.rawValue), object: nil)
        }
    }
    
    var favouriteOrders: [FavouriteOrder]? {
        didSet {
            NotificationCenter.default.post(name: Notification.Name(rawValue: JambaNotification.FavouriteOrdersUpdated.rawValue), object: nil)
        }
    }
    ...
    init(parseObject: PFObject) {
        ...
        // Push option
        if let pushNotifiationOption = parseObject["pushOptIn"] as? Bool{
            pushOptIn        =  pushNotifiationOption
        }else{
            pushOptIn = false
        }
        ...
        dateOfBirth      = parseObject["dateOfBirth"] as? Date
    ...
    func serializeAsParseObject() -> PFObject {
        ...
        // Save fishbowl push offer option
        parseObject["pushOptIn"] = pushOptIn
    ...
    init (user: User) {
        ...
        favouriteOrders = user.favouriteOrders
    }
    
    func updateUserFromSpendGoUser(_ spendGoUser: SpendGoUser) {
        id               = spendGoUser.id!
        phoneNumber      = spendGoUser.phoneNumber
        smsOptIn         = spendGoUser.smsOptIn
        emailAddress     = spendGoUser.emailAddress
        ...
        if spendGoUser.favoriteStore != nil {
            ...
            if existingStore != nil && existingStore!.storeCode == favoriteStore!.storeCode {
    ...
    func updateUserFromFishbowl(_ pushStatusOption:Bool){
        pushOptIn = pushStatusOption
    }
