            ...
            if let desc = pushNotificationResponse?.aps?.alert?.body{
                ...
                //if (verifyURL(urlString: desc) != nil){
                    ...
                    if let range = description.range(of: urlInString) {
                        ...
                        descriptionLabelMutableString  = NSMutableAttributedString(string: descriptionLabelString as String, attributes: [NSFontAttributeName:UIFont.init(name: "Helvetica Neue", size: 17.0)!])
                        
                        let textRange = NSMakeRange(startPos, (urlInString.characters.count))
                        descriptionLabelMutableString.addAttribute(NSForegroundColorAttributeName, value: linkBGColor, range: textRange)
                        
                        descriptionLabelMutableString.addAttribute(NSUnderlineStyleAttributeName , value: NSUnderlineStyle.styleSingle.rawValue, range: textRange)
                        
                        descriptionLabelMutableString.addAttribute(NSFontAttributeName, value: UIFont.italicSystemFont(ofSize: 17), range: textRange)
