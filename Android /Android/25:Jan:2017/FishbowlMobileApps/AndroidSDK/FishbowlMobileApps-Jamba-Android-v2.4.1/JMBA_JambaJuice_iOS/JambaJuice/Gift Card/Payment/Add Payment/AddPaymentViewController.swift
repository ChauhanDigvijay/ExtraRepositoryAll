...
class AddPaymentViewController: UIViewController,UITableViewDelegate,UITableViewDataSource, PickerViewControllerDelegate, UITextFieldDelegate, GiftCardWebviewDelegate, CardIOPaymentViewControllerDelegate, AddPaymentSingleTextFieldTableViewCellDelegate {
    ...
    func saveBillingDetails() {
        ...
        let cardNumber = (cardDetails[fieldName.cardNumber.hashValue - personalDetailsCount].field_value as NSString).substring(from: charCount)
    ...
    //get credit card image based on type
    func getCreditCardTypeImageUrl(_ creditCardtype:String) -> String {
    ...
    func getMonth(_ date:String) -> UInt16 {
        ...
        let month = UInt16(date.substring(to: index2!))!
        return month
    }
    func getYear(_ date:String) -> UInt16 {
        let str = date as NSString
        let year = UInt16(str.substring(with: NSRange(location: date.characters.count - 4, length: 4)))
        return year!
    }
    
    func getMonthInString(_ date:String) -> String {
        let index2 = date.range(of: " - ", options: .backwards)?.lowerBound
        let month:String = (date.substring(to: index2!))
        return month
    }
    func getYearInString(_ date:String) -> String {
        let str = date as NSString
        let year:String = (str.substring(with: NSRange(location: date.characters.count - 4, length: 4)))
        return year
    }
    ...
    func userDidProvide(_ cardInfo: CardIOCreditCardInfo!, in paymentViewController: CardIOPaymentViewController!) {
        ...
        if let info = cardInfo{
            ...
            let image = getCreditCardImageName("\(firstLetter)")
            cardDetails[fieldName.cardType.hashValue - personalDetails.count].field_value = getCreditCardType("\(firstLetter)")
            cardDetails[fieldName.cardType.hashValue - personalDetails.count].field_image = image
    ...
    func getCreditCardImageName(_ cardNumber:String) -> String {
        ...
        for cardType in brand!.creditCardTypesAndImages {
            if cardType.creditCardType.lowercased() == selectedCardType.lowercased() {
                return cardType.thumbnailImageUrl
            }
        }
        return ""
