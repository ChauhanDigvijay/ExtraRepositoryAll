...
public class FBRewardService {
    ...
    public static void getUserFBRewardPoint(final FBRewardPointCallback callback) {
        String path = "mobile/getPointRewardInfo" + "/" + FBPreferences.sharedInstance(fbSdk.context).getUserMemberforAppId();

        FBMainService.getInstance().get(path, null, new FBJsonServiceCallback() {
            @Override
            public void onServiceCallback(JSONObject response, Exception error) {
                parseResponseRewardpoint(response, error, callback);
            }
        });
    }
    ...
    private static void parseResponseRewardpoint(JSONObject response, Exception error, FBRewardPointCallback callback) {
        FBRewardPointDetailItem rewardpoint = null;
        if (response != null) {
            Gson gson = new Gson();
            try {
                String message = response.getString("message");
                Boolean successFlag = response.getBoolean("successFlag");
                int earnedPoints = response.getInt("earnedPoints");
                int pointsToNextReward = response.getInt("pointsToNextReward");

                rewardpoint = new FBRewardPointDetailItem(message, successFlag, earnedPoints,pointsToNextReward);
        ...
        if (callback != null) {
            callback.onFBRewardPointCallback(rewardpoint, error);
        }
    }
