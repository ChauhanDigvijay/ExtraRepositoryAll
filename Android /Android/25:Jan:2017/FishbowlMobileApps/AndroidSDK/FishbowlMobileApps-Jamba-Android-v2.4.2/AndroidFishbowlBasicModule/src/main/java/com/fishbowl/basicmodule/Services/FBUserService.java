...
/**
 * Created by Digvijay Chauhan //hbh
 */
...
public class FBUserService {
    ...
    public void getBonusRuleList(JSONArray parameter, final FBBonusRuleListCallback callback){
        ...
        String str  = parameter.toString();
        boolean dj=true;
        FBService.getInstance().makeCustomArrayRequest(FBConstant.FBBonusApi,null,getHeader1(),true, new FBServiceArrayCallback() {
            @Override
    ...
    public void getMenuProduct(JSONObject parameter,String storeId,String familyId,String categoryId,String subCategoryId,String productId,final FBMenuProductCallback callback){

        if (!FBUtility.isNetworkAvailable(fbSdk.context)) {
            return;
        }//storeId=647&familyId=1&categoryId=0&subCategoryId=1&productId=1
        String url = FBConstant.FBMenuProduct+"?storeId="+storeId+"& familyId="+familyId+"&categoryId="+categoryId+"&subCategoryId="+subCategoryId+"&productId="+productId;
        String str=parameter.toString();
        FBService.getInstance().get(url,null, menuCategoryHeader(), new FBServiceCallback(){
