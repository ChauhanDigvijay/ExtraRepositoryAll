...
class OloMenuServiceTests: OloTestCase {
    ...
    func helperTestProductModifiersOrOptions(_ shouldCheckModifiers: Bool) -> Void
    {
        let expectation = self.expectation(description: "OLOService")
        OloRestaurantService.restaurants { (restaurants, error) -> Void in
            if error != nil {
    ...
    fileprivate func helperTestModifier(_ modifier: OloModifier) -> Void {
...
//        XCTAssertFalse(modifier.maxSelects.isEmpty, "Modifier should have non-empty maxSelects")
    ...
    fileprivate func helperTestOption(_ option: OloOption) -> Void {
    ...
    fileprivate func helperTestCustomField(_ customField: OloCustomField) -> Void {
