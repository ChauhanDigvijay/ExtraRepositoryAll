...
open class OloUserService {
    ...
    open class func deleteCreditCard(billingAccountId:Int64!, _ callback: @escaping OloErrorCallback) {
        let authToken = OloSessionService.authToken
        if authToken == nil {
            callback(OloUtils.error("User is not authenticated.", code: 0))
            return
        }
        OloService.delete("/users/\(authToken!)/billingaccounts/\(billingAccountId!)", parameters: nil) { (response, error) -> Void in
            callback(error)
        }
    }
