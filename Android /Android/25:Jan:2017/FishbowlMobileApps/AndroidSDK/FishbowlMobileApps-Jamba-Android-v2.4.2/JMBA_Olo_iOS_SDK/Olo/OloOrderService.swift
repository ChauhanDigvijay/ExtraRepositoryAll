...
open class OloOrderService {
    ...
    open class func cancelOrder(_ orderGuid: String, callback: @escaping OloOrderStatusCallback) {
        let param:[String: Any] = [:]
        OloService.post("/orders/\(orderGuid)/cancel", parameters: param as OloJSONDictionary){ (response, error) -> Void in
            if error != nil {
                callback([], error)
            } else {
                let order = OloOrderStatus(json: response)
