...
class BasketOrderTimeTableViewCell: UITableViewCell {
    ...
    func updateButtonlabel() {
        ...
        let ASAPAttribute = [NSAttributedStringKey.font: ASAPFont, NSAttributedStringKey.foregroundColor: textColor]
        let ASAPAttributeString = NSMutableAttributedString(string:ASAPText, attributes:ASAPAttribute)
        let in15MinAttribute =  [NSAttributedStringKey.font:in15MinFont, NSAttributedStringKey.foregroundColor: textColor]
    ...
    @objc func asapSelected(_ button: SelectableButton) {
        log.verbose("Pickup day: ASAP")
        selectDay(.asap)
        delegate?.orderTimeCellDidChangeSelectedDay(self)
    }
    
    @objc func todaySelected(_ button: SelectableButton) {
        log.verbose("Pickup day: Today")
        selectDay(.today)
        delegate?.orderTimeCellDidChangeSelectedDay(self)
    }
    
    @objc func tomorrowSelected(_ button: SelectableButton) {
