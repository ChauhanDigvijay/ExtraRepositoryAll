...
typealias DeleteCreditCardCallback = (_ error: NSError?) -> Void
...
class BasketService: NSObject {
  ...
  /*  class func validateBasket(_ callback: @escaping BasketCallback) {
        ...
        OloBasketService.validate(sharedBasket!.id,true) { (result, error) -> Void in
            if error != nil {
                callback(nil, error)
                return
            }
    ...
    // Validation for upsell products
    class func updateTimeWantedAndValidate(_ callback: @escaping OloBasketValidationCallback) {
        updateTimeWanted { (basket, error) -> Void in
            if error != nil {
                callback(nil,error)
                return
            }
            self.updateTimeWantedValidateBasket { (basket, error) -> Void in
                if error != nil{
                    callback(nil,error)
                    return
                }
                return callback(basket,nil)
            }
        }
    }
    ...
    fileprivate class func placeOrderInternal(_ submitBody: OloBasketSubmitBody, callback: @escaping PlaceOrderCallback) {
        placeOrderInternalValidateBasket { (basket, error) -> Void in
            if error != nil {
                callback(nil, error)
                return
            }
...
//                AnalyticsService.trackEvent("order_ahead", action: "create_basket")
    ...
    class func deleteCreditCard(creditCardId: Int64!,callback:@escaping DeleteCreditCardCallback){
        OloUserService.deleteCreditCard(billingAccountId: creditCardId){(error) in
            if error != nil{
                return callback(error)
            } else {
                self.billingSchemes({ (basket, error) in
                    if error != nil{
                        return callback(error)
                    }else{
                        return callback(nil)
    ...
    //MARK: PlaceOrder Internal ValidateBasket
    class func placeOrderInternalValidateBasket(_ callback: @escaping BasketCallback){
        if sharedBasket == nil {
            callback(nil, NSError(domain: "Error", code: 100, userInfo: [NSLocalizedDescriptionKey:"Basket not found."]))
            return
        }
    ...
    //MARK: updateTimeWnated validate basket
    class func updateTimeWantedValidateBasket(_ callback: @escaping OloBasketValidationCallback){
        if sharedBasket == nil {
            callback(nil, NSError(domain: "Error", code: 100, userInfo: [NSLocalizedDescriptionKey:"Basket not found."]))
            return
        }
    ...
    //MARK: addUpSellProducts
    class func addUpSellProducts(oloUpSellRequestItems:OloUpSellRequestItems, callback:@escaping BasketCallback){
        
        var quantity = Int64(0)
        for upsellItem in oloUpSellRequestItems.items{
            quantity = Int64(quantity + upsellItem.quantity)
        }
        ...
        OloBasketService.addUpsell((sharedBasket?.id)!, oloUpSellRequestItems) { (oloBasket, error) in
            if error != nil {
                callback(nil, error)
                return
            }
