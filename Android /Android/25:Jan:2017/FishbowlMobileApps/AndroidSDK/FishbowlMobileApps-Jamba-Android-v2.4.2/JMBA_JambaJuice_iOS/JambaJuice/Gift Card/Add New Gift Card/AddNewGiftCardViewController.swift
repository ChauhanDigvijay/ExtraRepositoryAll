...
class AddNewGiftCardViewController: UIViewController,UITableViewDelegate,UITableViewDataSource, AmountSelectionViewControllerDelegate, PickerViewControllerDelegate, TemplateSelectionViewControllerDelegate, PurchaserDetailsViewControllerDelegate, RecepientsDetailViewControllerDelegate, AddPaymentViewControllerDelegate, PaymentViewControllerPassObjectDelegate,PaySafeWebViewDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == SegueIdentifiers.showPaymentListVC.rawValue {
            ...
            if (addNewCardCellDetails[addNewFieldName.selectCardAmount.hashValue].fieldRightSideValue.contains("$")) {
            ...
            let targetController = segue.destination as! TemplateSelectionViewController
            ...
            let targetController = segue.destination as! RecepientsDetailViewController
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if (addNewCardCellDetails[indexPath.row].fieldName == addNewFieldName.paymentAccount.rawValue) {
            let cell = tableView.dequeueReusableCell(withIdentifier: "AddNewGiftCardImageTableViewCell", for: indexPath) as! AddNewGiftCardImageTableViewCell
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "AddNewGiftCardViewCell", for: indexPath) as! AddNewGiftCardViewCell
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "AddGiftCardPreviewViewCell", for: indexPath) as! AddGiftCardPreviewViewCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if (addNewCardCellDetails[indexPath.row].fieldName == addNewFieldName.giftCardPreview.rawValue) {
            performSegue(withIdentifier: SegueIdentifiers.showTemplateSelectionVC.rawValue, sender: nil);
            ...
            if GiftCardCreationService.sharedInstance.purchaserDetails == nil {
                ...
                performSegue(withIdentifier: SegueIdentifiers.showRecepientsDetailVC.rawValue, sender: nil);
            }
    ...
    func selectedAmount(_ value:String) {
        ...
        var indexPath = IndexPath(row: addNewFieldName.selectCardAmount.hashValue, section: 0)
        ...
        indexPath = IndexPath(row: addNewFieldName.totalOrderSummary.hashValue, section: 0)
    ...
    func showQuantityPicker() {
        ...
        let indexPath = IndexPath(row: addNewFieldName.selectQuantity.hashValue, section: 0)
    ...
    func pickerValueChanged(_ value:String,index :Int) {
        ...
        var indexPath = IndexPath(row: addNewFieldName.selectQuantity.hashValue, section: 0)
        var cell = tableView.cellForRow(at: indexPath)
        if let cellType = cell as? AddNewGiftCardViewCell {
    ...
    func purchaserDetailSaved(_ purchaserDetails:InCommOrderPurchaser,purchaserDetailsFromPayment:Bool) {
        ...
        var indexPath = IndexPath(row: addNewFieldName.purchaserDetails.hashValue, section: 0)
        var cell = tableView.cellForRow(at: indexPath)
        if let cellType = cell as? AddNewGiftCardViewCell {
            let firstName = purchaserDetails.firstName
            let lastName  = purchaserDetails.lastName.first!
            let lastNameInitial = (String(describing: lastName)).uppercased()
            cellType.value.text = ("\(firstName) \(lastNameInitial).")
            cellType.value.adjustsFontSizeToFitWidth = true
            cellType.value.sizeToFit()
        }
        ...
        if let recipientDetail = GiftCardCreationService.sharedInstance.recipientDetails {
            indexPath = IndexPath(row: addNewFieldName.recipient.hashValue, section: 0)
            cell = tableView.cellForRow(at: indexPath)
            addNewCardCellDetails[addNewFieldName.recipient.hashValue].fieldRightSideValue = recipientDetail.emailAddress!
            if let cellType = cell as? AddNewGiftCardViewCell {
//                let firstName = recipientDetail.firstName
//                let lastName  = recipientDetail.lastName.first!
//                let lastNameInitial = (String(describing: lastName)).uppercased()
//                cellType.value.text = firstName + ", " + lastNameInitial
    ...
    func receipientDetailSaved(_ recipientDetails:InCommOrderRecipientDetails, message:String, recipientSelf:Bool) {
        ...
        let indexPath = IndexPath(row: addNewFieldName.recipient.hashValue, section: 0)
        let cell = tableView.cellForRow(at: indexPath)
        if let cellType = cell as? AddNewGiftCardViewCell {
//            let firstName = recipientDetails.firstName
//            let lastName  = recipientDetails.lastName.first!
//            let lastNameInitial = (String(describing: lastName)).uppercased()
//            cellType.value.text = firstName + ", " + lastNameInitial
    ...
    func paymentAdded(_ addedPaymentDetails:InCommSubmitPayment?, savePayment:InCommUserPaymentAccountDetails?, paymentDetailWithId: InCommSubmitPaymentWithId?, creditCardDetail: InCommUserPaymentAccount?, creditCardImage:String, cardLastDigits:String) {
        ...
        let indexPath = IndexPath(row: addNewFieldName.paymentAccount.hashValue, section: 0)
        ...
        if let cellType = cell as? AddNewGiftCardImageTableViewCell {
            ...
            if let url = URL(string: creditCardImage){
                ...
                cellType.creditCardTypeImage.af_setImage(withURL: url, placeholderImage: GiftCardAppConstants.jambaGiftCardDefaultImage, filter: nil, progress: nil, progressQueue: DispatchQueue.main, imageTransition: UIImageView.ImageTransition.noTransition, runImageTransitionIfCached: false, completion: { (response) in
    ...
    func initializeObject() {
        ...
        data.fieldRightSideValue = "Select"
        addNewCardCellDetails.append(data)
    ...
    func submitOrder(){
        ...
        SVProgressHUD.show(withStatus: "Purchasing Gift Card...")
                ...
                if error != nil{
                    ...
                    if let associateCardToUser = GiftCardCreationService.sharedInstance.recipientSelf {
                        if associateCardToUser {
                            ...
                            FishbowlApiClassService.sharedInstance.submitMobileAppEvent("GIFT_CARD_CREATED_SELF")
                        } else {
                            ...
                            FishbowlApiClassService.sharedInstance.submitMobileAppEvent("GIFT_CARD_CREATED_OTHERS")
                        }
                    }
    ...
    func associateGiftCards(){
        if associateGiftCardValidation(){
            SVProgressHUD.show(withStatus: "Associating Gift Card...")
    ...
    func removeAndVaildateUserSubmittedCard(_ index:Int){
        self.userSubmittedGiftCards.remove(at: index)
