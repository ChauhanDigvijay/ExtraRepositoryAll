...
class BasketViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, BasketOrderTimeTableViewCellDelegate, MGSwipeTableCellDelegate, BasketDeliveryModeTableViewCellDelegate, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        //Proceed order notification
        NotificationCenter.default.addObserver(self, selector: #selector(BasketViewController.proceedOrderFromUpsell), name: NSNotification.Name(rawValue: JambaNotification.ProceedOrder.rawValue), object: nil)
...
//        }
    ...
    func proceedOrder(){
        ...
        SVProgressHUD.setDefaultMaskType(.clear)
        // Validate upsell items need or not for user
        var noThanks:Bool = false
        if let sharedBasket = BasketService.sharedBasket{
            noThanks = sharedBasket.noThanks
        }
        
        if noThanks{
            self.proceedToBillingSchemes()
            return
        }
        BasketService.updateTimeWantedAndValidate { (basket, error) -> Void in
            if error != nil {
                SVProgressHUD.dismiss()
                self.presentError(error)
                return
            }
            // upSell Product validation
            if (basket?.upSellGroups.count != 0) {
                SVProgressHUD.dismiss()
                BasketService.sharedBasket?.updateBasketUpSellGroup(basket!)
                self.performSegue(withIdentifier: "UpSellProducts", sender: self)
            }else{
    ...
    // Proceed order from upsell product
  @objc  func proceedOrderFromUpsell(){
        self.proceedOrder()
    }
    
    // Proceed basket for billing schemes
    func proceedToBillingSchemes(){
        // Reset the nothanks option
