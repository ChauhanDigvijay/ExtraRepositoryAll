...
class MyOrdersTableViewController: UITableViewController {
    ...
    @objc func recentOrdersUpdated(_ notification: Notification) {
...
//        }
        if isOrderHistorySelected {
            ...
            if UserService.sharedUser!.favouriteOrders != nil && UserService.sharedUser!.favouriteOrders?.count > 0{
                ...
                let storeName = (favourite.vendorName as NSString).replacingOccurrences(of: "Jamba Juice ", with: "")
                cell.productLabel.text = storeName
                return cell
            }else{
                let cell = tableView.dequeueReusableCell(withIdentifier: "NoRecordsFound") as! MyOrderAndFavoriteEmptyTableViewCell
...
//                tableView.scrollEnabled = false
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if isOrderHistorySelected {
            ...
            if UserService.sharedUser!.favouriteOrders != nil && UserService.sharedUser!.favouriteOrders!.count > 0{
                ...
                self.performSegue(withIdentifier: "FavoriteOrderDetail", sender: nil)
            }
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        } else if segue.identifier == "FavoriteOrderDetail" {
            let vc = segue.destination as! FavouriteOrderDetailViewController
            if sender != nil{
                // vc.orderStatus = UserService.sharedUser!.recentOrders!.first
                vc.favourite = selectedFavouriteOrder
                vc.favourite.store = selectedFavouriteOrder?.store
            } else {
                vc.favourite = selectedFavouriteOrder
                vc.favourite.store = selectedFavouriteOrder?.store
            }
