...
class UserProfileTableViewController : UITableViewController, UITextFieldDelegate, PagedImagesViewControllerDelegate {
    ...
    @objc func userEmailChanged(_ notification: Notification) {
        populateEmailAddress()
    }
    
    @objc func userPhoneNumberChanged(_ notification: Notification) {
        if let user = UserService.sharedUser {
        phoneNumberLabel.text = user.phoneNumber
        }
    }
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        case .preferredStore:
            ...
            let textBoundingRect = (preferredStoreLabel.text as NSString?)?.boundingRect(with: size, options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font : preferredStoreLabel.font], context: nil)
    ...
    fileprivate func logout() {
        if BasketService.sharedBasket?.products.count > 0 {
            ...
            presentConfirmation("Hold on a second...", message: "Your existing basket will be lost upon logging out", buttonTitle: "Log out") { (confirmed) -> Void in
                if confirmed {
            ...
            presentConfirmation("Hold on a second...", message: "Are you sure you want to log out?", buttonTitle: "Log out") { (confirmed) -> Void in
                if confirmed {
                    self.logoutAndDismiss()
                }
            }
    ...
    @objc func showUserPreferredStore(){
       fetchUserUpdate()
    }
