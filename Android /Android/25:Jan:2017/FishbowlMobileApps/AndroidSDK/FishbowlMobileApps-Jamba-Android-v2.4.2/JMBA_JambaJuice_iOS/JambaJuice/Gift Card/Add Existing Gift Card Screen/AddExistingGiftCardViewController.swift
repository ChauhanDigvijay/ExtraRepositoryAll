...
class AddExistingGiftCardViewController: UIViewController,UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        if giftCardNumberTextField.text!.trim().isEmpty {
            giftCardNumberTextField.becomeFirstResponder()
        } else {
    ...
    @objc func keyBoardWillShown(_ notification:Notification){
        ...
        UIView.animate(withDuration: (userInfo.value(forKey: UIKeyboardAnimationDurationUserInfoKey) as! NSNumber).doubleValue, animations: {
            ...
            let edgInsets = UIEdgeInsetsMake(0.0, 0.0, keyboardFrame.cgRectValue.height - 150, 0.0)
            self.scrollView.contentInset = edgInsets
    ...
    @objc func keyBoardWillHidden(_ notification:Notification){
        ...
        UIView.animate(withDuration: (userInfo.value(forKey: UIKeyboardAnimationDurationUserInfoKey) as! NSNumber).doubleValue, animations: {
            ...
            let edgInsets = UIEdgeInsetsMake(0.0, 0.0, 0, 0.0)
            self.scrollView.contentInset = edgInsets
            self.view.layoutIfNeeded()
        }) 
    }
