...
class SignedInMainViewController: UIViewController{
    ...
    @objc func userNameUpdated(_ notification: Notification) {
        updateUserName()
    }
    
    @objc func userProfileImageUpdated(_ notification: Notification) {
        updateUserProfileImage()
    }
    
    @objc func orderStateChanged(_ notification: Notification) {
        //We just need to update recen order labels in case of no recent orders
        updateRecentOrders()
    }
    
    @objc func orderPlaced(_ notification: Notification) {
    ...
    @objc func updateRewardsAndOffers(){
    ...
    @objc func openStoreLoctionForShortCutItem(){
        let appdelegate = (UIApplication.shared.delegate) as! AppDelegate
        appdelegate.currentShortCutItem = JambaNotification.None
        closeModalScreen(){
            self.performSegue(withIdentifier: "StoreLocatorSegue", sender: self)
    ...
    @objc func openOrderMenuForShortCutItem(){
        let appdelegate = (UIApplication.shared.delegate) as! AppDelegate
        appdelegate.currentShortCutItem = JambaNotification.None
        closeModalScreen(){
