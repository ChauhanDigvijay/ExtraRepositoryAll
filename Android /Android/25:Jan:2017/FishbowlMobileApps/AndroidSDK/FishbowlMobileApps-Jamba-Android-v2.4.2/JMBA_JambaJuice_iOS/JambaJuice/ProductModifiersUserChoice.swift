...
struct ProductModifiersUserChoice {
    ...
    func isOptionIdSelected(_ optionId: Int64) -> Bool {
    ...
    func isOptionIdSelectedGetQuantity(_ optionId: Int64) -> Int {
        if optionId == selectedTypeOptionId {
            return 1
        }
        else if optionId == selectedSizeOptionId {
    ...
    func getTotalQuantity(_ modifierId: Int64) -> Int {
        var totalQuantity = 0
      ...
      //  return totalQuantity == 0 ? 0:totalQuantity - 1   //-1 for parent option
    }
    
    //get the total quantity selected by a user in a particular modifier without that option
    func getTotalQuantityWithOptionId(_ modifierId: Int64, optionId: Int64) -> Int {
        var totalQuantity = 0
        if selectedOptionIdsForModifierId[modifierId] == nil {
            return 0
        }
    ...
    //get the total quantity selected by a user in a particular modifier
    func updateQuantity(_ optionId: Int64, quantity:Int) -> Bool {
        for (_, selectedOptionIds) in selectedOptionIdsForModifierId {
            for optionIds in selectedOptionIds {
                if optionIds.choiceId == optionId {
    ...
    func getQuantityForAOption(_ optionId: Int64) -> Int {
        for (_, selectedOptionIds) in selectedOptionIdsForModifierId {
            for optionIds in selectedOptionIds {
                if optionIds.choiceId == optionId {
                    return optionIds.quantity
    ...
    mutating func addSelectedOptionIdForModifierId(_ modifierId: Int64, optionId: Int64) {
        if selectedOptionIdsForModifierId[modifierId] == nil {
    ...
    mutating func removeSelectedOptionIdForModifierId(_ modifierId: Int64, optionId: Int64) {
