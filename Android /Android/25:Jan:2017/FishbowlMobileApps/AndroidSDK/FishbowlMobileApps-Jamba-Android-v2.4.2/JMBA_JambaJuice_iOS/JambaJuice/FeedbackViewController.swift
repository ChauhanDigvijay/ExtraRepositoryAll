...
class FeedbackViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UITextViewDelegate {
    ...
    override func viewDidLoad() {
       ...
       navigationItem.title = feedback.feedbackType.rawValue
    }
    ...
    deinit {
        StatusBarStyleManager.popStyle(self)
         self.navigationController?.navigationBar.topItem?.title = feedback.feedbackType.rawValue
    }
    
    
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if (UserService.sharedUser == nil && indexPath.row == 2) || (UserService.sharedUser != nil && indexPath.row == 0){
            return 120
        }else{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return UserService.sharedUser == nil ? 3 : 1
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if !needsUserInfoCells && indexPath.row == 0 {
            caseNum = 2
        }
        switch caseNum {
        case 0:
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if UserService.sharedUser != nil {
            indexPathOfLastRow = IndexPath(row: 0, section: 0)
        }
        else {
            indexPathOfLastRow = IndexPath(row: 2, section: 0)
        }
    ...
    func validateFields() -> Bool {
        ...
        if !isLoggedIn {
           ...
           //Making Email Manitory and Check Valid Email Address
            if feedback.emailAddress == nil || (feedback.emailAddress?.trim().isEmpty)! {
                presentOkAlert("Email Required", message: "Please enter an email address") {
                    let cell = self.tableView.cellForRow(at: IndexPath.init(row: 1, section: 0)) as? FeedbackTextViewTableViewCell
                    cell?.textView.becomeFirstResponder()
                }
                return false
            } else {
                if !feedback.emailAddress!.isEmail() {
                    presentOkAlert("Invalid email address", message: "Please enter a valid email address") {
                        let cell = self.tableView.cellForRow(at: IndexPath.init(row: 0, section: 0)) as? FeedbackTextFieldTableViewCell
                        cell?.textField.becomeFirstResponder()
                    }
                    return false
                }
