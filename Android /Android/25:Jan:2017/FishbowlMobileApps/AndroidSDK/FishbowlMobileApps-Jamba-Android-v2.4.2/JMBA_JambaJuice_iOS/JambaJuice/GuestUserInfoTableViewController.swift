...
class GuestUserInfoTableViewController: UITableViewController, UITextFieldDelegate {
    ...
    func tryValidatingAndPopulateData() -> Bool {
        ...
        if validateFields() {
            let reference = UUID().uuidString // Random UUID
    ...
    func validateFields() -> Bool {
        ...
        if firstNameTextField.text == nil || firstNameTextField.text!.trim().isEmpty {
            ...
            presentOkAlert("First name required", message: "First name must be more than 3 characters") {
                self.firstNameTextField.becomeFirstResponder()
            }
            return false
        }
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        if textField == firstNameTextField {
            ...
            (parent as! GuestUserInfoViewController).continueToNextStep()
        }
        return true
    }
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if textField == phoneNumberTextField {
            ...
            if textField.shouldChangeInRange(range, replacementString: string, maxAllowedLength: Constants.oloFirstAndLastNameLimit){
