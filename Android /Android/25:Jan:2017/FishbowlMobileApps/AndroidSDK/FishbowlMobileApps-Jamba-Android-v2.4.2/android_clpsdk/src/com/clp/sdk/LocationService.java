...
public class LocationService extends Service implements ConnectionCallbacks,
		...
		public void onReceive(Context context, Intent intent) {
			try {
				...
				if (intent.hasExtra("EVENT") && intent.hasExtra("REGIONS")) {
					...
					if(_clpSdk.getClpSdkData().mobileSettings.TRIGGER_GEOFENCE>0){
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		try {
			...
			if (geoFencingCallbackReceiver != null) {
				IntentFilter intentFilter = new IntentFilter(CLPSdk.CLP_GEOFENCE_CALLBACK);
	...
	protected void startLocationUpdates() {
		...
		if(CLPSdk.sharedInstance(this).getIsLocationService()==true)
	...
	protected synchronized void buildGoogleApiClient() {
		...
		if(CLPSdk.sharedInstance(ctx)!=null){
	...
	public void onLocationChanged(Location location) {
		try {
			...
			//_clpSdk.displayLocalPushNotification("Location Update received",this.getClass(), R.drawable.ic_launcher, this);
			...
			if (isInsideRegion) {
				...
				if (insideRegionTime != null) {
					...
					if (regionEnteredtime >= (_clpSdk.getBEACON_SLAB_TIME())
							&& isInsideStore) {
						String inStore = isInStore(location);
						if (inStore != null) {
	...
	protected synchronized void startBeaconForStore() {
		...
		if (_clpSdk.allBeaconsForStores != null && this._clpSdk.getClpSdkData().mobileSettings.TRIGGER_BEACON > 0) {
