...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [_phoneNumberRadioButton setImage:[UIImage imageNamed:@"radio-enabled.png"] forState:UIControlStateNormal];
    [_newPhoneNumberRadioButton setImage:[UIImage imageNamed:@"radio-disabled.png"] forState:UIControlStateNormal];
    
    pleaseProvideInfoView.hidden=YES;
    searchOutlet.hidden=NO;
    self.navigationController.navigationBarHidden=YES;
    CustomBottomBarView *bottom1=[CustomBottomBarView sharedInst];
    _signInInfoLabel.hidden=YES;
    bottom1.hidden=YES;
    //  _phonNumberInfoView.hidden=NO;
...
-(void)showUser
{
    ...
    for (User *info in Userdata)
    {
        ...
        userEmailId=info.email_address;
        userPhoneNUm=info.phone_num;
