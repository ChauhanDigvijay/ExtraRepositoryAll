...
public class CustomExpandableListView implements View.OnClickListener {
    ...
    private int specialInstTextLength = 16;
    ...
    private View createGrandChildView(int groupPosition, final int childPosition, int grandChildPosition, boolean isLastChild) {
        ...
        if (DataManager.getInstance().getCurrentSelectedStore() != null) {
            specialInstTextLength = DataManager.getInstance().getCurrentSelectedStore().getSpecialinstructionsmaxlength();
        }
        InputFilter[] fArray = new InputFilter[1];
        fArray[0] = new InputFilter.LengthFilter(specialInstTextLength);
        specialInst.setFilters(fArray);
    ...
    private View createChildView(int groupPosition, final int childPosition, int subPosition, boolean isLastChild) {
        ...
        if (child.getSubOptions() != null) {
            ...
            if ((groupPosition + 1 == getGroupCount()) && isLastChild) {
                ...
                charLimit.setText(String.valueOf(specialInstTextLength - inst.length()));
...
//                        if (size > 0) {
//                            charLimit.setText(String.valueOf(specialInstTextLength - size));
//                        } else {
//                            charLimit.setText("");
//                        }
                        int size = specialInst.getText().toString().length();
