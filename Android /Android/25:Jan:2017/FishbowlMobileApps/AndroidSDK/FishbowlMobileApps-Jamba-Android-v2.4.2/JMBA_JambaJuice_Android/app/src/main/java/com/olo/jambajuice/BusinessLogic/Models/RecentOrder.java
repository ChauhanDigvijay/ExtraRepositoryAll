...
public class RecentOrder implements Serializable {
    ...
    public RecentOrder(JambaOrderStatus order) {
        products = new ArrayList<RecentOrderDetails>();
        if (order.getProducts() != null) {
            for (JambaOrderSatusProduct product : order.getProducts()) {
                products.add(new RecentOrderDetails(product));
            ...
            if (order.getProducts().size() > 0) {
                fofpName = (order.getProducts().size() == 1) ? order.getProducts().get(0).getName() : order.getProducts().get(0).getName() + ", ...";
            } else {
                fofpName = "-NO PRODUCTS-";
            }
        }
        total = order.getTotal();
        ...
        summary = new RecentOrderSummary(order,orderTimeStatement);
        id = order.getId();
        reference = order.getOrderref();
        vendorextref = order.getVendorextref();
        status = order.getStatus();
