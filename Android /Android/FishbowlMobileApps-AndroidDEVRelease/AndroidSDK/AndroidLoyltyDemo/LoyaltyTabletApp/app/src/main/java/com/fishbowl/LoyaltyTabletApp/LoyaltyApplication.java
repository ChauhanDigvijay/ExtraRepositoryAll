...
public class LoyaltyApplication extends Application implements Application.ActivityLifecycleCallbacks {
    ...
    private void initializeCLPSDK() {
        try {
            FBService.initialize(this, Constants.sdkPointingUrl(Constants.portalStaging), "null");
            FBCustomerItem customer = new FBCustomerItem();
            {
                ...
                LoyaltyApplication.instance.clpsdkObj = FBSdk.sharedInstance(this, Constants.sdkPointingUrl(Constants.portalStaging));
            }
        } catch (Exception e) {
            Log.i("Error ", "error");
        }
